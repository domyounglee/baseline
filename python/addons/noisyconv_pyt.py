...
class NoisyConvClassifier(ClassifierModelBase):
    ...
    def init_pool(self, dsz, **kwargs):
        filtsz = kwargs['filtsz']
        cmotsz = kwargs['cmotsz']
        self.parallel_conv = ParallelConv_nodropout(dsz, cmotsz, filtsz, "relu", self.pdrop)
    ...
    def pool(self, btc, lengths):
        ...
        self.context_vec = self.parallel_conv(embeddings)
        return  self.conv_drop( self.context_vec )
    ...
    def forward(self, input):
        ...
        self.logit = self.output(stacked)
        return self.logit

    def input_forward(self, input,d):
        # BxTxC
        embeddings = self.embed(input)
        pooled = self.pool(embeddings+d, input['lengths'])
        stacked = self.stacked(pooled)
